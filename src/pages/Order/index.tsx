...
export const Order = () => {
    ...
    const { register, handleSubmit, watch, formState: { errors } } = useForm(); // useForm 훅을 사용하여 폼 상태와 메소드 초기화

    // ref 대신 register 함수 사용
    const messageRef = register('message', { required: '카드 메시지를 입력해주세요.', maxLength: { value: 100, message: '카드 메시지는 100자 이내로 입력해주세요.' } });
    const receiptNumberRef = register('receiptNumber', { required: '현금영수증 번호를 입력해주세요.', pattern: { value: /^\d+$/, message: '현금영수증 번호는 숫자만 입력해주세요.' } });
        ...
        <ChakraProvider>
          <Flex direction={{ base: 'column', md: 'row' }}>
            <Box p={4} width={800} ml={200} mt={20}>
                ...
                />
                {errors.message && (
                    <Text color={"red"}>{(errors.message as FieldError)?.message}</Text>
                )}
